...
const ManageMemberships = () => {
  ...
  const handleSave = async (e) => {
    ...
    const payload = {
      ...
      classCredits: form.classCredits || null,
    };
    try {
      if (editing) {
      ...
      <div className="flex-1 p-4 md:p-8 overflow-auto">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {packages.map((p) => (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row gap-2 mt-auto">
                  <Button size="sm" variant="secondary" onClick={() => openEdit(p)} className="flex-1 w-full sm:w-auto">
                    Edit
                  </Button>
                  <Button size="sm" variant="danger" onClick={() => setDeleteModal(p)} className="flex-1 w-full sm:w-auto">
                    Delete
                  </Button>
                </div>
              </div>
            ...
            {!packages.length && (
              <p className="col-span-1 text-center py-12 text-gray-400">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input name="name" value={form.name} onChange={h} required className={S} />
              </div>

              <div className="col-span-2">
                <label className="label-base">Description</label>
                <input name="description" value={form.description} onChange={h} className={S} />
              </div>

              <div>
                <label className="label-base">Type *</label>
                <select name="type" value={form.type} onChange={h} className={S}>
                  {["monthly", "yearly", "class-pack", "drop-in"].map((t) => (
              ...
              <div>
                ...
                <input type="number" name="classCredits" value={form.classCredits} onChange={h} min="1" className={S} />
              </div>
              ...
              <div className="col-span-2">
                ...
                <textarea name="features" value={form.features} onChange={h} rows={4} className={`${S} resize-none`} />
              </div>
            </div>

            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <Button type="button" variant="secondary" onClick={() => setModal(false)} className="flex-1">
                Cancel
              </Button>
        ...
        <Modal isOpen={!!deleteModal} onClose={() => setDeleteModal(null)} title="Delete Package">
          <div className="text-center py-4">
            <p className="text-gray-700 font-medium mb-1">
              Delete <span className="text-primary">"{deleteModal?.name}"</span>?
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Button variant="danger" onClick={() => handleDelete(deleteModal?._id)} className="flex-1">
                Yes, Delete
              </Button>
            </div>
          </div>
